...
function Astrolabe:GetUnitPosition( unit, noMapChange )
	...
	if (x == nil or y == nil) then
		...
		-- or after WoW 7.1, in an instance where Blizzard doesn't want DBM monitoring position
		return;
	end
	if ( x <= 0 and y <= 0 ) then
		if ( noMapChange ) then
...
local function harvestMapData( HarvestedMapData, continentList )
	local mapData = {}
	local mapName = GetMapInfo();
	local mapID = GetCurrentMapAreaID();
	if ((not mapID) or HarvestedMapData[mapID]) then	-- multiple zones can use the same mapID (like Shadowmoon valley), don't keep redefining them
		return
	end
	local floorArray = { GetNumDungeonMapLevels() }	-- API change in WoW 7.0, now returns an array of values instead of a number
	local numFloors = 1
	if (floorArray ~= nil) then  --++ added to account for a nil return with patch 7.0.1
		numFloors = #floorArray
	end
	mapData.mapName = mapName;
	mapData.cont = (GetCurrentMapContinent()) or -100;
	mapData.zone = (GetCurrentMapZone()) or -100;
	mapData.numFloors = numFloors;
	if (continentList and continentList[mapData.cont]) then	-- only update continents we know about, rules out dungeons and most instances
		-- we're adding zones that are not already part of the continent list, so update the list of zones
		--print("updating continent: ", mapData.cont, mapID, mapData.mapName, continentList[mapData.cont] )
		local curZoneCount = # continentList[mapData.cont]	-- returns highest index used, not actual count
		continentList[mapData.cont][curZoneCount+1] = mapID
	end
...
local function activate( newInstance, oldInstance )
	...
	if not ( oldInstance and oldInstance.HarvestedMapData.VERSION == HARVESTED_DATA_VERSION ) then
		...
		local areaMaps = GetAreaMaps()
		for _, id in ipairs(areaMaps) do
			if not ( HarvestedMapData[id] ) then	-- NOTE: these are zone IDs, which are not always a 1:1 mapping to map IDs
				if ( SetMapByID(id) ) then
					harvestMapData(HarvestedMapData, newInstance.ContinentList);
				end
			end
		end
	end
...
for mapID, harvestedData in pairs(Astrolabe.HarvestedMapData) do
	...
	if( harvestedData.numFloors == nil ) then harvestedData.numFloors = 0; end
