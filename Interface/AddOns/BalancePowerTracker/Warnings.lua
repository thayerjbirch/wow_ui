...
local defaults = { 
	RealLunarEclipse = {
		...
		text = GetSpellInfo(171743),
		showTexture = true,
		texture = select(3,GetSpellInfo(171743)),
	},
	...
	RealSolarEclipse = {
		...
		text = GetSpellInfo(171744),
		showTexture = true,
		texture = select(3,GetSpellInfo(171744)),
	},
...
local function callback(energy, _, peak, _, _, virtual_peak, _)
	peak = peak or (abs(energy)==100 and energy)
	if peak then
	...
	elseif virtual_peak then
		--Predicted eclipse
		if warnedVEclipse ~= virtual_peak then
			warnEclipse(virtual_peak,1)
			warnedVEclipse = virtual_peak
			
			if getEnv("spellEffects",true) then
				if virtual_peak < 0 then
					SpellActivationOverlay_OnEvent(SpellActivationOverlayFrame,  "SPELL_ACTIVATION_OVERLAY_SHOW", 171743, "TEXTURES\\SPELLACTIVATIONOVERLAYS\\ECLIPSE_MOON.BLP", "Left", 1, 244, 244, 244)
				else
					SpellActivationOverlay_OnEvent(SpellActivationOverlayFrame,  "SPELL_ACTIVATION_OVERLAY_SHOW", 171744, "TEXTURES\\SPELLACTIVATIONOVERLAYS\\ECLIPSE_SUN.BLP", "Right (flipped)", 1, 244, 244, 244)
				end
			end
		end
	else
		if warnedVEclipse then
			if getEnv("spellEffects",true) then
				SpellActivationOverlay_OnEvent(SpellActivationOverlayFrame,  "SPELL_ACTIVATION_OVERLAY_HIDE", 171743)
				SpellActivationOverlay_OnEvent(SpellActivationOverlayFrame,  "SPELL_ACTIVATION_OVERLAY_HIDE", 171744)
			end
