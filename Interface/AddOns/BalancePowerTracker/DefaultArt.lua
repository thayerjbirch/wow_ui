...
local function EclipseBar_LunarEclipse() 
	...
	local glowInfo = { x=43, y=45, left=0.73437500, right=0.90234375, top=0.00781250, bottom=0.35937500 } ;
...
local function EclipseBar_SolarEclipse()
	me.benergy:ClearAllPoints();
	local glowInfo =  { x=43, y=45, left=0.55859375, right=0.72656250, top=0.00781250, bottom=0.35937500 };
...
-- Recalc
local energy,direction,peak,vEnergy,vDirection,vPeak = 0,"moon",false,0,"moon",false
local range = 38
...
local function recalc()
	...
	elseif getEnv("predict",true) and vPeak then
		if warnedEclipse ~= vPeak then
			warnEclipse(vPeak)
			warnedEclipse = vPeak
		end		
	else
...
function me.LoadModule(getOptionValueFunction,r)
	...
	me.callbackId = LibBalancePowerTracker:RegisterCallback(function(e, d, p, ve, vd, vp)
		energy,direction,peak,vEnergy,vDirection,vPeak = e*0.005, d, p, ve*0.005, vd, vp
		recalc();
	end);
	me.loaded = true
end
