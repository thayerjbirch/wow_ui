...
local GetSpellCooldown,type,pairs,select,print,GetSpellInfo,GetSpecialization = GetSpellCooldown,type,pairs,select,print,GetSpellInfo,GetSpecialization
...
function BalancePowerTracker.isBalance()
	return (GetSpecialization() == 1) and (class == "DRUID");
end
...
function SlashCmdList.BALANCEPOWERTRACKER(msg, editbox) --Slash command function
	local loaded,reason = LoadAddOn("BalancePowerTracker_Options") 
				
	if loaded ~= 1 and reason then
